...
root = tk.Tk(className="carre_control_ui")
root.title("carre_control_ui")
...
def pose_callback(data):
    print(data)
    x_p.set("{0:.2f}".format(data.x))
    y_p.set("{0:.2f}".format(data.y))
    z_p.set("{0:.2f}".format(data.z))

# def rot_callback(data):
#     z_o.set("{0:.2f}".format( math.degrees(quaterionToRads(data)) ))

rospy.init_node('carre_control_ui', anonymous=False)
#Subscribers
posicionLider_sub = rospy.Subscriber("/carre_sim/odom/twist/twist/linear", Odometry , pose_callback)
...
vel_pub = rospy.Publisher('/carre_sim/cmd_vel', Twist, queue_size=1)
...
ttk.Button(mainframe, text="Stop", command=stop).grid(column=2, row=6, sticky=tk.W)
